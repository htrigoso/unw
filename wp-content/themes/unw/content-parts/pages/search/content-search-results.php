...
$external_links = [
  [
    ...
    'url'      => 'https://librodereclamaciones.uwiener.edu.pe/?_gl=1*jxf9yi*_gcl_au*MjU3MzU1MTk5LjE3NTc0NTc3MzI.',
    'content'  => 'Accede a nuestro Libro de Reclamaciones en línea.',
  ],

];

// Recorremos los enlaces externos
if (!empty($search_term)) {
  foreach ($external_links as $link) {
    foreach ($link['keywords'] as $kw) {
      if (stripos($search_term, $kw) !== false) {
        $results[] = [
          "title"   => $link['title'],
          "href"    => $link['url'],
          "content" => $link['content'],
        ];
        break 2; // rompe ambos loops después de inyectar
      }
    }
  }
}
      ...
      <?php if (!empty($search_term)): ?>
        <?php if (!empty($results)): ?>
          <?php foreach ($results as $result): ?>
            <a href="<?php echo esc_url($result['href']); ?>" target="<?php echo strpos($result['href'], 'http') === 0 ? '_blank' : '_self'; ?>" rel="<?php echo strpos($result['href'], 'http') === 0 ? 'noopener' : ''; ?>">
