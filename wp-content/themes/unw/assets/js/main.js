document.addEventListener('DOMContentLoaded', function () {
  ...
  const observer = new IntersectionObserver((entries) => {
    ...
    const currentSection = visibleSections.reduce((prev, current) => {
      return prev.target.offsetTop > current.target.offsetTop ? prev : current
    })

    const targetId = '#' + currentSection.target.id
    const activeLink = document.querySelector(`.tabs_menu .link_item_tab[href="${targetId}"]`)
  ...
  const scrollToHash = () => {
    ...
    if (hash) {
      ...
      if (targetElement) {
        ...
        const activeLink = document.querySelector(`.tabs_menu .link_item_tab[href="${hash}"]`)
        if (activeLink) {
          navLinks.forEach((l) => l.classList.remove('w--current'))
          activeLink.classList.add('w--current')
        }
